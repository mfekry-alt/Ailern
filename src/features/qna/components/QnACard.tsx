...
function VoteBox({
  ...
  return (
    ...
    >
      <span className="hidden sm:block text-[9px] font-black uppercase tracking-widest mb-1">
        Vote
      </span>
      <div
        className={`p-1 sm:p-1.5 rounded-lg sm:rounded-full ${votedByMe ? 'bg-blue-100 text-blue-600 dark:bg-blue-500/20' : 'bg-slate-200/50 text-slate-400 dark:bg-slate-700'}`}
      >
...
export function QnACard({
  ...
  const shouldShowDelete = (() => {
    // Rule 1: If instructor or admin, always show delete
    if (isInstructor || hasRole('Admin')) {
      return true;
    }

    // Rule 2: If student, check if they are the owner
    if (user && q.author.role === 'student') {
      // Compare user.id with q.author.id (use loose equality to handle string/number mismatch)
      return (user.id == q.author.id) && !q.hasInstructorReply ;
    }

    // Rule 3: Otherwise, hide
    return false;
  })();
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div className="flex items-center gap-0.5 sm:gap-1">
                {isInstructor && (
                  ...
                  >
                    <Pin
                      className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${q.isPinned ? 'fill-current' : ''}`}
                    />
                  </button>
                )}
