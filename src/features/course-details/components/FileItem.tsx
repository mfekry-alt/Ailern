...
const getFileIcon = (contentType: string) => {
    ...
    if (contentType.toLowerCase().includes('zip') || contentType.toLowerCase().includes('rar') || contentType.toLowerCase().includes('archive'))
        return Archive;
    if (contentType.toLowerCase().includes('pdf') || contentType.toLowerCase().includes('document') || contentType.toLowerCase().includes('text'))
        return FileText;
    return File;
};
            ...
            <div className="flex items-center gap-4 flex-1 min-w-0">
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                        ...
                        {isVideo && (
                            <span className="px-1.5 py-0.5 rounded-md bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 text-[8px] font-black uppercase tracking-widest border border-amber-100 dark:border-amber-500/20">Video</span>
                        )}
                    </div>
                    <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-[10px] text-indigo-500 dark:text-indigo-400 font-black uppercase tracking-wider opacity-90">Ready to Learn</span>
                    </div>
                </div>
            </div>
