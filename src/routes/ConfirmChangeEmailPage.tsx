...
import { confirmChangeEmail } from '@/api/services/auth.service';
...
export const ConfirmChangeEmailPage = () => {
    ...
    useEffect(() => {
        ...
        const verify = async () => {
            try {
                await confirmChangeEmail({ userId, newEmail, token });
                setStatus('success');
                // Notify other tabs (like the original ChangeEmailPage)
                localStorage.setItem('emailConfirmed', 'true');
                
                // Log out after showing the success message for a brief moment
                setTimeout(() => {
        ...
        verify();
    }, [userId, newEmail, token, logoutMutation]);
                    ...
                    <div className="flex flex-col items-center py-10 gap-5">
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-sm text-slate-500 dark:text-zinc-400 max-w-xs mx-auto">
                                Your new email has been verified successfully. Redirecting you to sign in...
                            </p>
                        </div>
                        <Link
                            to={ROUTES.LOGIN}
