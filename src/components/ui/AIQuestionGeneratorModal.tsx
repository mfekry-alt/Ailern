...
import { ArrowLeft, Upload, FolderOpen, Sparkles, FileText, X, ChevronDown } from 'lucide-react';
...
export function AIQuestionGeneratorModal({ isOpen, quizId, onClose, onGenerate }: AIQuestionGeneratorModalProps) {
    ...
    const [selectedSection, setSelectedSection] = useState<string>(''); // Empty = all sections
    ...
    const availableFiles = useMemo(() => {
        ...
        if (selectedSection && sectionsQuery.data) {
            const section = sectionsQuery.data.find(s => s.id === selectedSection);
            if (section) {
                // For now, filter by section title pattern or return all (section-level filtering would require API enhancement)
                // Client-side filter: if file name contains section title, include it
                return files.filter(f =>
                    f.fileName.toLowerCase().includes(section.title.toLowerCase()) ||
                    f.fileName.toLowerCase().includes(`section ${section.sectionNumber}`)
                );
            }
        }

        return files;
    }, [generationFilesQuery.data, selectedSection, sectionsQuery.data]);
    ...
    const handleGenerate = () => {
        ...
        const payload: GenerateQuizByAIPayload = {
            ...
            questionTypeCounts: {
                MCQ: mcqCount,
                TrueFalse: Math.max(0, totalQuestions - mcqCount - writtenCount),
                            ...
                            <div className="rounded-xl border border-zinc-800 bg-zinc-900/40 p-3 space-y-3">
                                ...
                                {sectionsQuery.data && sectionsQuery.data.length > 0 && (
                                    <div className="relative">
                                        <label className="block text-xs text-zinc-500 mb-1.5">Filter by Section (Optional)</label>
                                        <div className="relative">
                                            <select
                                                value={selectedSection}
                                                onChange={(e) => setSelectedSection(e.target.value)}
                                                className="w-full appearance-none bg-zinc-900 border border-zinc-700 rounded-lg px-3 py-2 pr-8 text-xs text-zinc-200 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 outline-none cursor-pointer"
                                            >
                                ...
                                {quizId && !generationFilesQuery.isLoading && availableFiles.length === 0 && (
                                ...
                                {availableFiles.length > 0 && selectedMaterials.length === 0 && (
