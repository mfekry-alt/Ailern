...
export const startOrResumeQuizAttempt = async (quizId: string): Promise<StartAttemptResponse> => {
    // 1. أولاً: نفحص بقوة هل هناك محاولة InProgress فعلاً؟
    ...
    if (activeAttempt && activeAttempt.id) {
        console.log('💡 Found existing active attempt, resuming:', activeAttempt.id);
        return activeAttempt;
    }

    // 2. ثانياً: إذا لم تكن هناك محاولة، نبدأ واحدة جديدة
    try {
        const response = await api.post(ENDPOINTS.ATTEMPTS.START(quizId), {});
        return normalizeAttempt(response.data?.data || response.data);
    } catch (error: any) {
        // إذا حدث خطأ (مثل 400)، نحاول مرة أخرى لعل السيرفر أنشأها في الخلفية
